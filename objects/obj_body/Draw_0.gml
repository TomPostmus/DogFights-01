// Update trunk hit mask
with (hitmask_trunk) {
	x = other.get_x()
	y = other.get_y()
	image_angle = other.get_trunk_rotation()
}

// Update head hit mask
with (hitmask_head) {
	x = other.get_head_x()
	y = other.get_head_y()
	image_angle = other.get_head_rotation()
}

// Update arm hitmasks
if (instance_exists(character) && instance_exists(character.weapon)) {
	var arm_anim_component = character.weapon.anim_component_arm_left
	with (hitmask_arm_left) {
		x = arm_anim_component.get_x(other)
		y = arm_anim_component.get_y(other)
		image_angle = arm_anim_component.get_arm_rotation(other)
		image_index = arm_anim_component.get_arm_index(other)
	}

	arm_anim_component = character.weapon.anim_component_arm_right
	with (hitmask_arm_right) {
		x = arm_anim_component.get_x(other)
		y = arm_anim_component.get_y(other)
		image_angle = arm_anim_component.get_arm_rotation(other)
		image_index = arm_anim_component.get_arm_index(other)
	}
}

// Draw self
appearance.draw(id)