...
/// @param recoil_y
/// @req_sprite spr_usp
function dog_anim_usp_idle() {

	var dog = argument[0];
	var recoil_x = argument[1];
	var recoil_y = argument[2];
	var this_script = dog_anim_usp_idle;
	...
	with (dog) {
		...
		switch (anim_arms_index) {
			case 0: ds_list_add(anim_draw_order, arm_left); ds_list_add(anim_draw_order, arm_right); ds_list_add(anim_draw_order, weapon); 
				...
				weapon.pos_x = 21.40 - recoil_x; weapon.pos_y = 5.60 + recoil_y; weapon.rotation = 0; weapon.sprite = spr_usp; weapon.index = 0; weapon.is_visible = true
